...
export class HotelSphereDB extends Dexie {
  ...
  constructor() {
    ...
    // Version bumped to 8 to reflect final table additions
    ...
    const tablesToSync = [
      ...
      'shiftLogs', 'cleaningLogs', 'quotations'
    ];
      ...
      if (table) {
        table.hook('creating', (primKey: any, obj: any) => {
          Dexie.currentTransaction?.on('complete', () => {
            if (obj && (obj.id || primKey)) pushToCloud(tableName, obj);
          });
        });
        ...
        table.hook('updating', (mods: any, primKey: any, obj: any) => {
          Dexie.currentTransaction?.on('complete', () => {
            ...
            if (merged && merged.id) pushToCloud(tableName, merged);
          });
        });
      }
    });
...
export async function exportDatabase() {
  const tables = [
    'rooms', 'guests', 'bookings', 'transactions', 'settings', 'groups', 'supervisors', 
    'banquetHalls', 'eventBookings', 'cateringMenu', 'restaurants', 'menuItems', 
    'diningTables', 'kots', 'diningBills', 'inventory', 'vendors', 'facilityUsage', 
  ...
  a.href = url;
  a.download = `hotelsphere_pro_backup_${new Date().toISOString().split('T')[0]}.json`;
  a.click();
}
