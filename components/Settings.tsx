...
import { HostelSettings, Room, RoomStatus, Booking, Transaction, Supervisor, UserRole, RoomTypeRate } from '../types.ts';
...
const Settings: React.FC<SettingsProps> = ({ 
  ...
  const [activeSubTab, setActiveSubTab] = useState<'GENERAL' | 'ROOMS' | 'STAFF' | 'DATA' | 'TAX' | 'GUEST_PORTAL' | 'MEAL_PLANS' | 'OCCUPANCY'>('GENERAL');
  ...
  const handleMealRateUpdate = (plan: string, rate: number) => {
    const currentRates = tempSettings.mealPlanRates || {};
    handleUpdate('mealPlanRates', { ...currentRates, [plan]: rate });
  };

  const handleOccupancyRateUpdate = (roomType: string, field: keyof RoomTypeRate, value: number) => {
    const currentRates = tempSettings.roomTypeRates || {};
    const typeRate = currentRates[roomType] || { single: 0, double: 0 };
    handleUpdate('roomTypeRates', { 
      ...currentRates, 
      [roomType]: { ...typeRate, [field]: value } 
    });
  };
  ...
  return (
    <div className="p-4 md:p-8 bg-[#f8fafc] min-h-full pb-32 text-black overflow-x-hidden">
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="flex items-center bg-white p-2 md:p-3 rounded-2xl md:rounded-[2rem] border shadow-xl sticky top-2 z-10 overflow-x-auto scrollbar-hide no-print gap-1">
          ...
          <SubTab active={activeSubTab === 'OCCUPANCY'} label="Occupancy Rates" onClick={() => setActiveSubTab('OCCUPANCY')} />
          <SubTab active={activeSubTab === 'MEAL_PLANS'} label="Meal Plans" onClick={() => setActiveSubTab('MEAL_PLANS')} />
          ...
          <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
             <section className="bg-white p-8 md:p-12 rounded-[3.5rem] border shadow-sm space-y-10">
                <div className="border-b pb-6">
                   <h3 className="text-3xl font-black text-blue-900 uppercase tracking-tighter">Occupancy Rate Matrix</h3>
                   <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Set rates for Single vs Double occupancy for each room type</p>
                </div>
                
                <div className="space-y-6">
                   {tempSettings.roomTypes.map(type => (
                      <div key={type} className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex flex-col md:flex-row items-center gap-8 group hover:border-blue-200 transition-all">
                         <div className="flex-1">
                            <h4 className="text-xl font-black text-blue-900 uppercase tracking-tight">{type}</h4>
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Pricing Logic</p>
                         </div>
                         <div className="grid grid-cols-2 gap-4 w-full md:w-auto">
                            <Input 
                               label="Single Occupancy (₹)" 
                               type="number" 
                               value={tempSettings.roomTypeRates?.[type]?.single?.toString() || '0'} 
                               onChange={v => handleOccupancyRateUpdate(type, 'single', parseFloat(v) || 0)} 
                            />
                            <Input 
                               label="Double Occupancy (₹)" 
                               type="number" 
                               value={tempSettings.roomTypeRates?.[type]?.double?.toString() || '0'} 
                               onChange={v => handleOccupancyRateUpdate(type, 'double', parseFloat(v) || 0)} 
                            />
                         </div>
                      </div>
                   ))}
                ...
                <div className="p-6 bg-blue-50 border border-blue-100 rounded-3xl flex gap-6 items-start">
                   ...
                   <p className="text-[11px] font-bold text-blue-900 leading-relaxed uppercase">
                      Configuring single vs double rates allows you to easily switch pricing during check-in based on guest count. These values will be suggested by the system automatically.
                   </p>
                </div>
             </section>
          </div>
        ...
        {activeSubTab === 'MEAL_PLANS' && (
          <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
             <section className="bg-white p-8 md:p-12 rounded-[3.5rem] border shadow-sm space-y-10">
                <div className="border-b pb-6">
                   <h3 className="text-3xl font-black text-blue-900 uppercase tracking-tighter">Meal Plan Pricing</h3>
                   <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Set daily per-room rates for different packages</p>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <Input label="EP Rate (Room Only)" type="number" value={tempSettings.mealPlanRates?.['EP (Room Only)']?.toString() || '0'} onChange={v => handleMealRateUpdate('EP (Room Only)', parseFloat(v) || 0)} />
                   <Input label="CP Rate (Room + B/Fast)" type="number" value={tempSettings.mealPlanRates?.['CP (Room + B/Fast)']?.toString() || '500'} onChange={v => handleMealRateUpdate('CP (Room + B/Fast)', parseFloat(v) || 0)} />
                   <Input label="MAP Rate (Room + 2 Meals)" type="number" value={tempSettings.mealPlanRates?.['MAP (Room + 2 Meals)']?.toString() || '1200'} onChange={v => handleMealRateUpdate('MAP (Room + 2 Meals)', parseFloat(v) || 0)} />
                   <Input label="AP Rate (Room + All Meals)" type="number" value={tempSettings.mealPlanRates?.['AP (Room + All Meals)']?.toString() || '1800'} onChange={v => handleMealRateUpdate('AP (Room + All Meals)', parseFloat(v) || 0)} />
                </div>

                <div className="p-6 bg-blue-50 border border-blue-100 rounded-3xl flex gap-6 items-start">
                   <div className="text-2xl mt-1">🍽️</div>
                   <p className="text-[11px] font-bold text-blue-900 leading-relaxed uppercase">
                      These rates will be automatically fetched during Check-in and Reservation when you select a plan. You can still manually override them on the check-in screen if needed.
                   </p>
                </div>
             </section>
          </div>
          ...
          <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
             ...
             <section className="bg-white p-6 md:p-10 rounded-3xl border shadow-sm space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-6 gap-6 items-end">
                   ...
                   <Input label="Base Rate (₹)" type="number" value={newRoom.price} onChange={v => setNewRoom({...newRoom, price: parseFloat(v)})} />
