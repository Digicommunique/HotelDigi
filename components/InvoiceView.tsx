...
const InvoiceView: React.FC<InvoiceViewProps> = ({ guest, booking, groupBookings, room, settings, payments }) => {
  const isVip = booking?.isVip || groupBookings?.some(b => b.isVip);
  const isInclusive = booking?.isGstInclusive || groupBookings?.some(b => b.isGstInclusive);
  ...
  const totalTaxRate = settings.taxRate || 0;
  ...
  const lineItems = renderBookings.map(b => {
    ...
    // VIP stays have zero rent
    const dailyBase = b.isVip ? 0 : (b.basePrice || 0);
    const dailyMeal = b.isVip ? 0 : (b.mealRate || 0);
    const rent = (dailyBase + dailyMeal) * nights;
    
    totalRoomRent += rent;
    totalDiscount += b.isVip ? 0 : (b.discount || 0);

    if (b.charges) {
      b.charges.forEach(c => {
        allCharges.push({ ...c, roomNo: b.roomNumber });
        if (!b.isVip) totalServiceCharges += c.amount;
      });
    }

    return { ...b, nights, dailyBase, dailyMeal, rent };
  });

  const grossTotal = totalRoomRent + totalServiceCharges;
  const taxableSumRaw = grossTotal - totalDiscount;
  
  let taxableAmount = 0;
  let totalTax = 0;

  if (isInclusive) {
    taxableAmount = taxableSumRaw / (1 + (totalTaxRate / 100));
    totalTax = taxableSumRaw - taxableAmount;
  } else {
    taxableAmount = taxableSumRaw;
    totalTax = (taxableAmount * totalTaxRate) / 100;
  }
    ...
    <div className="bg-white p-10 w-[210mm] min-h-[297mm] mx-auto text-[11px] text-gray-800 font-sans leading-tight border border-gray-200 shadow-2xl print:border-none print:shadow-none print:m-0 print:p-6 invoice-sheet">
      ...
      <div className="flex justify-between items-start mb-8 border-b-4 border-blue-900 pb-6">
        ...
        <div className="text-right flex flex-col justify-between h-24">
          <div className={`px-6 py-2 rounded-xl inline-block shadow-lg ${isVip ? 'bg-amber-600' : 'bg-blue-900'} text-white`}>
             <p className="text-[10px] font-black uppercase tracking-widest">
               {isVip ? 'VIP COMPLIMENTARY' : (groupBookings ? 'Consolidated Invoice' : 'Tax Invoice')}
             </p>
          </div>
      ...
      <div className="bg-gray-50 border p-5 rounded-2xl mb-8 flex justify-between shadow-sm">
        ...
        <div className="text-right space-y-3">
          ...
          {isInclusive && <div className="text-[8px] font-black text-blue-600 uppercase mt-1">Rates are GST Inclusive</div>}
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-blue-50/50 border-b border-gray-200 font-black uppercase text-blue-900 text-[9px]">
            <tr>
              ...
              <th className="p-4 text-right">Room Tariff</th>
              <th className="p-4 text-right">Meal Rate</th>
            ...
            {lineItems.map((item, idx) => (
              <tr key={idx} className="bg-white">
                <td className="p-4">
                   ...
                   {item.isVip && <span className="text-[7px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded uppercase font-black">VIP Protocol</span>}
                </td>
                <td className="p-4 text-center">{item.nights}</td>
                <td className="p-4 text-right">₹{item.dailyBase.toFixed(2)}</td>
                <td className="p-4 text-right">₹{item.dailyMeal.toFixed(2)}</td>
        ...
        <div className="border border-gray-200 rounded-2xl overflow-hidden mb-8 shadow-sm">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-gray-200 font-black uppercase text-slate-500 text-[9px]">
              <tr>
                <th className="p-4">Modular Services</th>
            ...
            <tbody className="divide-y divide-gray-100 font-bold uppercase text-gray-600">
              {allCharges.map((charge, idx) => (
                <tr key={idx} className={`bg-white ${isVip ? 'opacity-30' : ''}`}>
                  <td className="p-4 truncate max-w-xs">{charge.description} {isVip && '(VIP Waived)'}</td>
      ...
      <div className="grid grid-cols-2 gap-10 mb-8">
        <div className="space-y-4">
           <div className="bg-gray-50 border p-4 rounded-2xl">
              ...
              <div className="space-y-2 font-bold text-[10px] uppercase">
                 ...
                 <div className="flex justify-between text-[9px] text-blue-700 italic">
                    <span>GST Status</span>
                    <span>{isInclusive ? 'Included in Base' : 'Added to Base'}</span>
                 </div>
              </div>
           </div>
           {!isVip && (
        ...
        <div className={`rounded-[2rem] p-8 text-white space-y-3 shadow-xl transition-all ${isVip ? 'bg-amber-600' : 'bg-blue-900'}`}>
           ...
           {isVip && (
             <div className="text-center bg-white/20 p-2 rounded-xl border border-white/10 text-[9px] font-black uppercase mt-4">
                VIP Complimentary Privilege applied
             </div>
           )}
        </div>
      </div>
