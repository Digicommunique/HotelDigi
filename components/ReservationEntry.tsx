...
const ReservationEntry: React.FC<ReservationEntryProps> = ({ onClose, existingGuests, rooms, onSave, settings }) => {
  ...
  const handleSearchGuest = () => {
    ...
    if (found) {
      ...
      setAdults(found.adults?.toString() || '1');
      setChildren(found.children?.toString() || '0');
      setKids(found.kids?.toString() || '0');
    } else {
  ...
  const totalOccupantsCount = useMemo(() => {
    return (parseInt(adults) || 0) + (parseInt(children) || 0) + (parseInt(kids) || 0);
  }, [adults, children, kids]);

  const currentMealRateForFolio = useMemo(() => {
    const ratePerPerson = settings.mealPlanRates?.[mealPlan] || 0;
    return ratePerPerson * totalOccupantsCount;
  }, [mealPlan, totalOccupantsCount, settings.mealPlanRates]);

  const currentEstimatedTariffPerRoom = useMemo(() => {
    if (selectedRoomIds.length === 0) return 0;
    const firstRoom = rooms.find(r => r.id === selectedRoomIds[0]);
    if (!firstRoom) return 0;
    const roomBase = settings.roomTypeRates?.[firstRoom.type]?.[occupancy] || firstRoom.price || 0;
    return roomBase + currentMealRateForFolio;
  }, [selectedRoomIds, rooms, settings.roomTypeRates, occupancy, currentMealRateForFolio]);
  ...
  const handleSave = () => {
    ...
    const bookings = selectedRoomIds.map(roomId => {
      ...
      const roomBase = settings.roomTypeRates?.[r?.type || '']?.[occupancy] || r?.price || 0;
      ...
      return {
        ...
        mealPlan,
        mealRate: currentMealRateForFolio,
        agent: bookingAgent,
          ...
          <div className="w-[380px] border-r bg-slate-50/50 p-8 overflow-y-auto custom-scrollbar space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1">
                 ...
                 <select className="w-full border-2 p-3 rounded-2xl text-[12px] font-black bg-white outline-none" value={gender} onChange={e => setGender(e.target.value as any)}>
            ...
            <div className="space-y-4">
               <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <select className="w-full border-2 p-3 rounded-2xl text-[12px] font-black bg-white focus:border-orange-500 outline-none" value={mealPlan} onChange={e => setMealPlan(e.target.value)}>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-slate-400 ml-1">Occupancy</label>
                    <select className="w-full border-2 p-3 rounded-2xl text-[12px] font-black bg-white focus:border-orange-500 outline-none" value={occupancy} onChange={e => setOccupancy(e.target.value as any)}>
               ...
               <div className="grid grid-cols-3 gap-2">
                  <Inp label="Adult" type="number" value={adults} onChange={setAdults} isCompact />
                  <Inp label="Child" type="number" value={children} onChange={setChildren} isCompact />
                  <Inp label="Kid" type="number" value={kids} onChange={setKids} isCompact />
               </div>
            </div>
          </div>
          ...
          <div className="flex-1 p-10 space-y-8 overflow-y-auto custom-scrollbar bg-white">
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="flex flex-col items-center justify-center gap-4 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2.5rem] p-6">
                ...
                <button type="button" onClick={() => { setActiveDocCapture({type:'photo', isSecondary:false}); setIsCameraOpen(true); }} className="bg-orange-50 text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase">Capture Guest</button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-6 bg-orange-50/50 p-8 rounded-[3rem] border border-orange-100">
               ...
               <div className="space-y-1">
                 ...
                 <select className="w-full border-2 p-3 rounded-2xl text-[12px] font-black bg-white outline-none" value={advanceMethod} onChange={e => setAdvanceMethod(e.target.value)}>
            ...
            <div className="p-6 bg-blue-50 border border-blue-100 rounded-3xl mt-4 space-y-2">
               <div className="flex justify-between">
                  <p className="text-[10px] font-bold text-blue-900 uppercase">Meal Rate (Total for {totalOccupantsCount} Pers.):</p>
                  <p className="text-[12px] font-black text-blue-900">₹{currentMealRateForFolio.toFixed(2)}</p>
               </div>
               <div className="flex justify-between border-t border-blue-100 pt-2">
                  <p className="text-[10px] font-bold text-blue-900 uppercase">Estimated Total (per room/night):</p>
                  <p className="text-xl font-black text-blue-900">₹{currentEstimatedTariffPerRoom.toFixed(2)}</p>
               </div>
            </div>
          </div>
...
const Inp = ({ label, value, onChange, type = "text", isCompact = false }: any) => (
  <div className="space-y-1.5 w-full text-left">
    <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">{label}</label>
    <input 
      type={type} 
      className={`w-full border-2 ${isCompact ? 'p-2 md:p-3 text-center' : 'p-3'} rounded-2xl font-black text-[12px] text-black bg-white focus:border-orange-500 transition-all outline-none leading-none appearance-none`} 
      value={value || ''} 
