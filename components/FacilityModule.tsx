...
const FacilityModule: React.FC<FacilityModuleProps> = ({ guests, bookings, rooms, settings, onUpdateBooking }) => {
  ...
  useEffect(() => {
    const init = async () => {
       ...
       const savedConfig = await db.settings.get('facility_config');
       if (savedConfig) setConfig(savedConfig as any);
       
       const savedPkgsData = await db.settings.get('facility_packages');
       if (savedPkgsData && (savedPkgsData as any).packages) {
         setPackages((savedPkgsData as any).packages);
       } else {
         setPackages(INITIAL_PACKAGES);
         await db.settings.put({ id: 'facility_packages', packages: INITIAL_PACKAGES } as any);
       }
    };
    init();
  }, []);
  ...
  const handleSavePackage = async () => {
    ...
    setPackages(updated);
    await db.settings.put({ id: 'facility_packages', packages: updated } as any);
    setEditingPkg(null);
    ...
    <div className="p-8 h-full flex flex-col gap-8 bg-[#f8fafc]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 flex-1 overflow-hidden">
        ...
        <div className="bg-[#003d80] text-white rounded-[3.5rem] p-10 shadow-2xl flex flex-col gap-8 no-print overflow-y-auto custom-scrollbar">
           ...
           <div className="space-y-6 flex-1 pr-1">
                ...
                <div className="space-y-6">
                   ...
                   {formData.bookingId && (
                     <div className="p-6 bg-white/10 border border-white/20 rounded-3xl animate-in slide-in-from-top-4 duration-300 shadow-inner">
                        <p className="text-[9px] font-black uppercase text-blue-300 tracking-widest mb-4">Resident Card</p>
                        <div className="space-y-3">
                           <div className="flex justify-between items-center">
                              <span className="text-[9px] font-bold text-white/40 uppercase">Room</span>
                              <span className="text-lg font-black text-white">{formData.roomNumber}</span>
                           </div>
                           <div className="flex justify-between items-center">
                              <span className="text-[9px] font-bold text-white/40 uppercase">Name</span>
              ...
              {activeTab !== 'LAUNDRY' && (
                 <div className="space-y-6 pt-6 border-t border-white/5">
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase text-white/70 ml-2 tracking-widest">Plan</label>
                        <select 
                          className="w-full bg-white/5 border-2 border-white/10 p-4 rounded-2xl font-black text-xs text-white outline-none focus:bg-white/10" 
                    ...
                    <SidebarInp label="Discount (₹)" type="number" value={formData.discount.toString()} onChange={(v: string) => setFormData({...formData, discount: parseFloat(v) || 0})} />
                    ...
                    {guestType === 'RESIDENT' && (
                       <label className="flex items-center justify-between p-5 bg-white/5 rounded-3xl cursor-pointer border-2 border-transparent hover:border-white/20 transition-all">
                          <span className="text-[10px] font-black uppercase">Post to Room Folio</span>
                          <input type="checkbox" checked={formData.assignToRoom} onChange={e => setFormData({...formData, assignToRoom: e.target.checked})} className="w-6 h-6 rounded-xl accent-blue-600" />
                       </label>
                    )}
                 </div>
              )}
           ...
           <button onClick={handleAuthorize} className="w-full bg-white text-blue-900 py-6 rounded-[2rem] font-black uppercase text-xs shadow-2xl hover:bg-blue-50 transition-all hover:scale-[1.02]">
              {activeTab === 'LAUNDRY' ? 'Commit Order 👕' : 'Authorize Entry ⚡'}
           </button>
        </div>
      </div>
      ...
      {editingPkg && (
        <div className="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
           <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300 text-slate-900">
              <div className="bg-blue-900 p-8 text-white flex justify-between items-center">
                 <h3 className="text-xl font-black uppercase tracking-tighter">{editingPkg.id ? 'Edit Plan' : 'New Plan'}</h3>
              ...
              <div className="p-10 space-y-6">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        ...
                        <select className="w-full border-2 p-3.5 rounded-2xl font-bold text-xs bg-slate-50" value={editingPkg.facilityType} onChange={e => setEditingPkg({...editingPkg, facilityType: e.target.value as any})}>
                 ...
                 <TimeInp label="Validity / Unit" value={editingPkg.validity || ''} onChange={(v: string) => setEditingPkg({...editingPkg, validity: v})} placeholder="e.g. 1 Day, Per Item" />
                 <button onClick={handleSavePackage} className="w-full bg-blue-900 text-white py-5 rounded-2xl font-black uppercase text-xs">Commit</button>
              </div>
           </div>
        </div>
      )}
...
const StatCard = ({ label, value, icon, color }: any) => (
   <div className={`${color} p-8 rounded-[3rem] text-white shadow-xl relative overflow-hidden group`}>
      ...
      <p className="text-[10px] font-black uppercase opacity-60 mb-3">{label}</p>
...
const SubTab = ({ active, label, onClick }: any) => (
  <button onClick={onClick} className={`px-8 py-3 rounded-2xl transition-all font-black text-[10px] uppercase whitespace-nowrap ${active ? 'bg-[#003d80] text-white shadow-xl' : 'text-slate-400 hover:text-blue-900 hover:bg-white'}`}>{label}</button>
);
...
const TimeInp = ({ label, value, onChange, type = "text", placeholder = "" }: any) => (
   <div className="space-y-1 w-full text-left">
      ...
      <input type={type} className="w-full border-2 p-3.5 rounded-2xl font-bold text-xs bg-slate-50 outline-none focus:bg-white text-black" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} />
   </div>
);

const SidebarInp = ({ label, value, onChange, type = "text", placeholder = "" }: any) => (
  <div className="space-y-1.5">
     <label className="text-[10px] font-black uppercase text-white/70 ml-2">{label}</label>
     <input 
        type={type} 
        className="w-full bg-white/5 border-2 border-white/10 p-4 rounded-2xl font-black text-xs outline-none focus:bg-white/10 text-white placeholder:text-white/20 shadow-inner" 
        value={value} 
